 ...
 */
public class PieYearFragment extends Fragment implements
		OnChartValueSelectedListener {

	private PieChart mChart;// 饼图控件
	private TextView noDataTV;// 无支出提醒

	public static final int CHART_PIE_YEAR = 1;
	...
	public View onCreateView(LayoutInflater inflater,
		...
		View view = inflater.inflate(R.layout.fragment_year_pie, container,
				false);
		return view;
	}
	...
	private void initView() {
		mChart = (PieChart) getView().findViewById(R.id.pie_year_chart);
		noDataTV = (TextView) getView().findViewById(R.id.pie_year_no_data_tv);
	}
		...
		selectCallBack = new ISelectExpendByTypeCB() {
			...
			public void selectSuccess(final List<ExpendByType> expendList) {
				handler.post(new Runnable() {
					...
					@Override
					public void run() {
						if (getActivity() instanceof ExpendPieAnalysisActivity) {
							((ExpendPieAnalysisActivity) getActivity())
									.isRefreshOvew(CHART_PIE_YEAR);
						}
						dialog.dismiss();
				...
				handler.post(new Runnable() {
					...
					public void run() {
						...
						case ISelectExpendByTypeCB.NET_ERROR: {
							ToastUtils.showToast("获取超时，请确认网络连接后再试");
							break;
						}
						default:
							break;
